...
2. **DEG Construction**: An even-regular graph (`SizeBoundedGraph`) is constructed using DEG's `EvenRegularGraphBuilder` with the `EVP_InnerProduct` metric for fast quantized distance computation.
