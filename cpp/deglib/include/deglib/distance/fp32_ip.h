...
DEGLIB_TARGET_AVX512 inline static float fp32_hsum512(__m512 s) { return _mm512_reduce_add_ps(s); }
  ...
  public:
    ...
    DEGLIB_TARGET_AVX512 inline static float compare(const void* pVect1v, const void* pVect2v, const void* qty_ptr) {
        ...
        if constexpr (HasSimd) {
            __m512 q_vec = _mm512_loadu_ps(a);
            __m512 db_vec = _mm512_loadu_ps(b);
            sum512_1 = _mm512_fmadd_ps(q_vec, db_vec, sum512_1);
            a += 16;
            b += 16;
        }
            ...
            if constexpr (HasDualSimd) {
                ...
                for (size_t c = 0; c < nc32; ++c) {
                    ...
                    for (size_t j = 0; j < BATCH_SIZE; ++j) {
                        ...
                        __m512 db_vec_lo = _mm512_loadu_ps(&db_[idx]);
                        __m512 db_vec_hi = _mm512_loadu_ps(&db_[idx + 16]);
                        s[j] = _mm512_fmadd_ps(q_vec_lo, db_vec_lo, s[j]);
                        s[j] = _mm512_fmadd_ps(q_vec_hi, db_vec_hi, s[j]);
                    }
                }
            }
        ...
        auto batch_impl = [query, dim](const void* const* db, float* out_dists) DEGLIB_TARGET_AVX2 {
            ...
            if constexpr (HasDualSimd) {
                ...
                for (size_t c = 0; c < nc16; ++c) {
                    ...
                    for (size_t j = 0; j < BATCH_SIZE; ++j) {
                        const float* db_ = static_cast<const float*>(db[j]);
                        __m256 db_vec_lo = _mm256_loadu_ps(&db_[idx]);
                        __m256 db_vec_hi = _mm256_loadu_ps(&db_[idx + 8]);
                        s[j] = _mm256_fmadd_ps(q_vec_lo, db_vec_lo, s[j]);
                        s[j] = _mm256_fmadd_ps(q_vec_hi, db_vec_hi, s[j]);
                    }
                }
            }
    ...
    if (target == deglib::cpu::InstructionSet::AVX512 || target == deglib::cpu::InstructionSet::AVX512_VNNI) {
        if (dim < 16) {
            ...
            if (rem == 0)
                ...
                return InnerProductFloat_AVX512<ResidualMode::DualTail>{};
            else
        ...
        if (dim < 8) {
            ...
            if (rem == 0)
                ...
                return InnerProductFloat_AVX2<ResidualMode::DualTail>{};
            else
