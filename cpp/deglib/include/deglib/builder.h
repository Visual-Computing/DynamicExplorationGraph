...
struct BuilderAddTask {
    ...
    BuilderAddTask(uint32_t lbl, uint64_t index, std::vector<std::byte> feat) : label(lbl), manipulation_index(index), feature(std::move(feat)) {}
};
...
struct BuilderChange {
    ...
    BuilderChange(uint32_t internalIdx, uint32_t fromIdx, float fromWeight, uint32_t toIdx, float toWeight)
        : internal_index(internalIdx), from_neighbor_index(fromIdx), from_neighbor_weight(fromWeight), to_neighbor_index(toIdx), to_neighbor_weight(toWeight) {}
};
...
class EvenRegularGraphBuilder {
  ...
  private:
    ...
    void extendGraphUnknownLID(const BuilderAddTask& add_task) {
        ...
        auto top_list = graph.search_intern(
            entry_vertex_indices, new_vertex_feature, std::max(uint32_t(this->extend_k_), edges_per_vertex * 2), this->extend_eps_
        );  // need 2x otherwise it might lock during neighbor selection
        ...
        while (slots > 0) {
            for (size_t i = 0; i < candidates.size() && slots > 0; i++) {
                ...
                uint32_t new_neighbor_index = 0;
                {
                    ...
                    // this should not be possible, otherwise the new vertex is connected to every vertex in the neighbor-list of the candidate-vertex and still
                    // has space for more
        ...
        isolated_vertices.erase(
            std::remove_if(isolated_vertices.begin(), isolated_vertices.end(), [&graph](int val) { return graph.hasEdge(val, val) == false; }),
    ...
    void extendGraphKnownLID(const std::span<const BuilderAddTask> add_tasks) {
        ...
        auto process_task = [&](const TaskWithIndex& item) {
            ...
            while (new_neighbors.size() < edges_per_vertex) {
                for (size_t i = 0; i < results.size() && new_neighbors.size() < edges_per_vertex; i++) {
                    ...
                    if (this->optimizationTarget_ == HighLID) {
                        ...
                        // should not be possible, otherwise the new vertex is connected to every vertex in the neighbor-list of the result-vertex and still has
                        // space for more
                        if (new_neighbor_weight == std::numeric_limits<float>::lowest()) continue;
                    ...
                    // this should not be possible, otherwise the new vertex is connected to every vertex in the neighbor-list of the result-vertex and still
                    // has space for more
            ...
            if (new_neighbors.size() < edges_per_vertex) {
                std::fprintf(
                    stderr, "could find only %zu good neighbors for the new vertex %u need %u\n", new_neighbors.size(), internal_index, edges_per_vertex
                );
                std::perror("");
                std::abort();
            }
        ...
        deglib::concurrent::parallel_for(0, task_count, extend_thread_count, [&](size_t task_index, size_t thread_id) {
            ...
            for (size_t i = start_index; i < end_index; ++i) process_task(tasks_with_indices[i]);
        });
    }
    ...
    void restoreGraph(const std::vector<uint32_t>& involved_indices, bool improve_edges) {
        ...
        auto unique_groups = std::vector<std::shared_ptr<ReachableGroup>>();
        {
            ...
            auto is_enough_free_connections = [](const std::vector<uint32_t>& vertices, const UnionFind& paths,
        ...
        for (const auto& group : unique_groups)
            if (group->size() == 1) isolated_groups.push_back(group);
        ...
        if (improve_edges && this->improve_k_ > 0) {
            ...
            auto compareByWeight = [](const GraphEdge& a, const GraphEdge& b) {
                if (a.weight != b.weight) return a.weight > b.weight;  // < is ascending, > is descending
                if (a.from_vertex != b.from_vertex) return a.from_vertex < b.from_vertex;
    ...
    bool improveEdges() {
        ...
        if (bad_neighbor_weight != std::numeric_limits<float>::lowest()) success |= improveEdges(vertex1, bad_neighbor_index, bad_neighbor_weight);
  ...
  public:
    ...
    BuilderStatus build(std::function<void(deglib::builder::BuilderStatus&)> callback, const bool infinite = false) {
        ...
        do {
            ...
            if (this->new_entry_queue_.size() > 0 || this->remove_entry_queue_.size() > 0) {
                ...
                if (this->remove_entry_queue_.size() > 0) del_task_manipulation_index = this->remove_entry_queue_.front().manipulation_index;
     ...
     */
    BuilderStatus build() { return build(nullptr, false); }
