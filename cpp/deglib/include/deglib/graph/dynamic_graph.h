...
class DynamicGraph : public deglib::graph::MutableGraph {
 ...
 public:
   ...
   */
  DynamicGraph(const uint8_t edges_per_vertex, const deglib::distances::FloatSpace feature_space, std::ifstream& ifstream, const uint32_t size, const uint32_t chunk_size = 1024)
   ...
   * @param chunk_size Target number of vertices per memory chunk (rounded up to nearest power of 2, default = 1024).
   */
  static DynamicGraph create_empty(
  ...
  uint32_t capacity() const {
    return static_cast<uint32_t>(chunks_.size()) * chunk_capacity_;
  }
  ...
  const uint32_t size() const override {
    return static_cast<uint32_t>(this->label_to_index_.size());
  }
 ...
 public:
  ...
  const bool saveGraph(const char* path_to_graph) const override {
    ...
    for (uint32_t i = 0; i < graph_size; i++)
      out.write(reinterpret_cast<const char*>(this->vertex_by_index(i)), file_byte_size_per_vertex);
    out.close();
  ...
  bool changeEdge(const uint32_t internal_index, const uint32_t replace_index, const uint32_t new_index, const float new_weight) override {
    ...
    if (replace_pos == neighbor_indices_end || *replace_pos != replace_index) {
      std::cerr << "changeEdge: vertex " << internal_index << " does not have an edge to " << replace_index
                << " and therefore can not be swapped with " << new_index << " and distance " << new_weight << std::endl;
      return false;
    }
 ...
 * @param chunk_size Target number of vertices per memory chunk (rounded up to nearest power of 2, default = 1024).
 */
inline auto convert_to_dynamic_graph(const deglib::graph::InternalGraph& input_graph, const deglib::distances::FloatSpace feature_space, const void* custom_features = nullptr, const uint32_t chunk_size = 1024)
{
