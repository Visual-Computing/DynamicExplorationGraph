...
class VisitedListPool {
  ...
  public:
    VisitedListPool(uint32_t initmaxpools, uint32_t numelements) : num_elements_(numelements) {
        for (uint32_t i = 0; i < initmaxpools; i++) pool_.push_back(new VisitedList(numelements));
    }
    ...
    class FreeVisitedList {
      ...
      public:
        ...
        ~FreeVisitedList() noexcept { pool_.releaseVisitedList(list_); }

        auto operator->() const { return &list_; }
    };
