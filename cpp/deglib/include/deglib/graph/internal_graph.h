...
class InternalGraph {
  ...
  static DEGLIB_TARGET_AVX512_VNNI deglib::search::LinearPool<float> searchEfImplAvx512VnniD200(
       ...
       deglib::distances::int8_ip::InnerProductInt8_AVX512_VNNI_D200::QueryState qs;
       deglib::distances::int8_ip::InnerProductInt8_AVX512_VNNI_D200::prepare_query(reinterpret_cast<const int8_t*>(query), qs);
        ...
        for (size_t i = 0; i < entry_vertex_indices.size(); ++i) {
           ...
           if (ep < vertex_count) {
               ...
               float distance = deglib::distances::int8_ip::InnerProductInt8_AVX512_VNNI_D200::compare_prepared(feat, qs);
               if (distance < dist1) {
                   dist1 = distance;
                   ep1 = ep;
               }
       ...
       if (ep1 < vertex_count) {
        ...
        while (pool.has_next()) {
            ...
            for (int32_t i = 0; i < edge_size; ++i) {
               ...
               float dist = deglib::distances::int8_ip::InnerProductInt8_AVX512_VNNI_D200::compare_prepared(feat, qs);
