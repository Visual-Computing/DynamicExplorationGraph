...
class PositionLocks {
  public:
    explicit PositionLocks(int count) {
        ...
        for (int i = 0; i < pool_size_; i++) flags_[static_cast<size_t>(i)].clear(std::memory_order_relaxed);
    }
    ...
    inline bool try_lock_positions(const int* positions, int n) {
        for (int i = 0; i < n; i++) {
            ...
            if (flag->test_and_set(std::memory_order_acquire)) {
                for (int k = 0; k < i; k++) flags_[static_cast<size_t>(positions[k] % pool_size_)].clear(std::memory_order_release);
                return false;
            }
        }
        return true;
    ...
    inline void unlock_positions(const int* positions, int n) {
        for (int i = 0; i < n; i++) flags_[static_cast<size_t>(positions[i] % pool_size_)].clear(std::memory_order_release);
    }
...
class SwapThreadPool {
  public:
    explicit SwapThreadPool(int num_threads) : n_(num_threads < 1 ? 1 : num_threads), bar_(n_, ResetGo{&go_}) {
        for (int t = 1; t < n_; ++t) workers_.emplace_back([this, t] { worker_main(t); });
    }
  ...
  private:
    ...
    void worker_main(int tid) {
        for (;;) {
            ...
            std::function<void(size_t, size_t)> fn;
            {
                ...
                cv_.wait(lk, [&] { return stop_.load(std::memory_order_acquire) || go_.load(std::memory_order_acquire); });
    ...
    static void run_chunk(const Fn& fn, int tid, size_t end, int n) {
        ...
        size_t start = (static_cast<size_t>(tid) < rem) ? static_cast<size_t>(tid) * (base + 1) : rem * (base + 1) + (static_cast<size_t>(tid) - rem) * base;
        size_t len = base + (static_cast<size_t>(tid) < rem ? 1 : 0);
        for (size_t i = 0; i < len; ++i) fn(start + i, static_cast<size_t>(tid));
    }
...
inline void do_sorting_1d(
    ...
    if (callback && callback(0.f)) return;
    ...
    do {
        ...
        pool.parallel_for(static_cast<size_t>(count), [&](size_t i, size_t /*tid*/) {
            std::copy_n(ctx.map_fields[static_cast<int>(i)].feature, static_cast<size_t>(dim), grid.row(static_cast<int>(i), dim));
        });
        ...
        for (int i = 0; i < settings.num_filters; i++) filter_weighted_som_1d(radius_1d, ctx, grid);
        ...
        pool.parallel_for(static_cast<size_t>(num_threads), [&](size_t t, size_t) {
            ...
            for (int i = 0; i < num_swap_indices; i++) worker.swaps.swap_indices[i] = i;
        ...
        if (num_swap_tries > 1) {
            pool.parallel_for(static_cast<size_t>(num_swap_tries), [&](size_t /*try_id*/, size_t thread_id) {
