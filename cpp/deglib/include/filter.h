...
public:
    // Constructor: valid labels as int-pointer and size, max_value defines the maximum label value, max_label_count is the total possible
    // labels
    ...
    size_t size() const { return current_valid_count_; }
    ...
    double get_inclusion_rate() const { return static_cast<double>(current_valid_count_) / max_label_count_; }
};
