...
public:
    ...
    explicit JVScratch(int dim)
        : dim_(dim),
          capacity_(dim),
          buffer_(static_cast<size_t>(kNumRegions) * static_cast<size_t>(dim), 0) {}
    ...
    // zero is sufficient.
    void reset() noexcept {
        std::fill(buffer_.begin(), buffer_.begin() + static_cast<size_t>(kNumRegions) * static_cast<size_t>(dim_), 0);
    }
