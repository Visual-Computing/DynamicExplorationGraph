...
public:
    StaticFeatureRepository(std::unique_ptr<std::byte[]> contiguous_features,
                            const size_t dims,
                            const size_t count,
                            const size_t bytes_per_dim)
        : bytes_per_dim_{bytes_per_dim},
          dims_{dims},
          count_{count},
          contiguous_features_{
              std::move(contiguous_features),
          } {}
...
inline auto fvecs_read(const char* fname, size_t& d_out, size_t& n_out) {
    ...
    for (size_t i = 0; i < n; i++)
        std::memmove(&x[i * dims * sizeof(float)], &x[sizeof(dims) + i * (dims + 1) * sizeof(float)], dims * sizeof(float));
