...
private:
    ...
    void restoreGraph(const std::vector<uint32_t>& involved_indices, bool improve_edges) {
        ...
        auto unique_groups = std::unordered_set<std::shared_ptr<ReachableGroup>>();
        {
            ...
            auto not_enough_free_connections = [](const std::vector<uint32_t>& vertices,
                                                  const UnionFind& paths,
                                                  const std::unordered_map<uint32_t, std::shared_ptr<ReachableGroup>>& groups) {
            ...
            if (this->use_path_verification_) {
                ...
                for (const auto involved_index : involved_indices) {
                    ...
                    auto group_a = reachable_groups.at(root_a);
                    ...
                    for (uint32_t i = 0; i < edges_per_vertex; i++) {
                        ...
                        if (std::binary_search(sorted_involved.begin(), sorted_involved.end(), neighbor_index)) {
                            ...
                            if (root_a != root_b) {
                                ...
                                group_a->copyFrom(*reachable_groups.at(root_b));
                            }
                        }
                    }
                }
                ...
                while (not_enough_free_connections(involved_indices, path_map, reachable_groups)) {
                    for (auto it = reachable_groups.begin(); it != reachable_groups.end(); ++it) {
                        ...
                        if (group->size() <= 1) {
                            ...
                            std::vector<deglib::search::ObjectDistance> traceback =
                                graph.hasPath(from_indices, group->getVertexIndex(), ext_eps, ext_k);
                ...
                if (not_enough_free_connections(involved_indices, path_map, reachable_groups)) {
                    std::fprintf(stderr, "restoreGraph: not enough free connections to reconnect isolated vertices\n");
                    std::perror("");
                    abort();
                }
            } else {
                ...
                while (not_enough_free_connections(involved_indices, path_map, reachable_groups)) {
                    for (const auto check_vertex : check) {
                        ...
                        if (neighbor_check_depth > 0 && reachable_group->size() > 1) continue;
                        ...
                        for (uint32_t i = 0; i < edges_per_vertex; i++) {
                            ...
                            if (neighbor_index == check_vertex) continue;
            ...
            for (const auto involved_index : involved_indices) unique_groups.emplace(reachable_groups.at(path_map.Find(involved_index)));
        }
        ...
        for (const auto& group : unique_groups) {
            if (group->size() == 1) isolated_groups.emplace_back(group);
        }
        ...
        for (const auto& isolated_group : isolated_groups) {
            ...
            if (isolated_group->size() > 1) continue;
            ...
            for (const auto candidate_group : unique_groups) {
                ...
                if (missing_edges.size() <= 2) continue;
        ...
        auto compareBySize = [](const std::shared_ptr<deglib::builder::ReachableGroup>& a,
                                const std::shared_ptr<deglib::builder::ReachableGroup>& b) {
            ...
            for (const auto reachable_index : reachable_vertices) {
                ...
                for (auto other_it = other_vertices.begin(); other_it != other_vertices.end(); ++other_it) {
                    ...
                    // Deterministic tie-breaking
                        ...
                        if (reachable_index < best_reachable_index ||
                            (reachable_index == best_reachable_index && other_index < best_other_index)) {
                            best_reachable_index = reachable_index;
                            best_other_index = other_index;
                        }
                    }
        ...
        auto remaining_indices =
            std::vector<uint32_t>(reachable_groups[0]->getMissingEdges().begin(), reachable_groups[0]->getMissingEdges().end());
        ...
        // 3.5 the remaining vertices can not be connected to any of the other involved vertices, because they already have an edge to all
        // of them.
        for (size_t i = 0; i < remaining_indices.size(); i++) {
            const auto index_A = remaining_indices[i];
            if (graph.hasEdge(index_A, index_A)) {  // still missing an edge?
        ...
        if (improve_edges && this->improve_k_ > 0) {
            ...
            for (size_t i = 0; i < new_edges.size(); i++) {
                ...
                if (graph.hasEdge(edge.from_vertex, edge.to_vertex)) improveEdges(edge.from_vertex, edge.to_vertex, edge.weight);
            }
        }
    }
     ...
     */
    bool improveEdges(std::vector<deglib::builder::BuilderChange>& changes,
                      uint32_t vertex1,
                      uint32_t vertex2,
                      uint32_t vertex3,
                      uint32_t vertex4,
...

        {
            ...
            //    their subgraph and would therefore connect the two potential subgraphs.
            {
                ...
                // Sometimes the gain between the two best combinations is the same, its better to use one with the bad edges to make later
                // improvements easier
                for (auto&& result : topListDescending(top_list)) {
                    ...
                    if (vertex1 != new_vertex3 && vertex2 != new_vertex3 && graph.hasEdge(vertex2, new_vertex3) == false) {
                        ...
                        //     Furthermore Vertex 3 has now to many edges, find an good edge to remove to improve the overall graph
                        //     distortion. FYI: If the just selected vertex3 is the same as the old vertex3, this process might cut its
                        //     connection to vertex4 again. This will be fixed in the next step or until the recursion reaches
                        //     max_path_length.
                ...
                if (dist23 == std::numeric_limits<float>::lowest()) return false;
            ...
            if (vertex1 == vertex4) {
                    ...
                    if (vertex4 != good_vertex && graph.hasEdge(vertex4, good_vertex) == false) {
                        ...
                        for (size_t i = 0; i < edges_per_vertex; i++) {
                            ...
                            if (vertex4 != selected_neighbor && graph.hasEdge(vertex4, selected_neighbor) == false) {
                                ...
                                const auto new_neighbor_dist =
                                    dist_func(vertex4_feature, graph.getFeatureVector(selected_neighbor), dist_func_param);
                ...
                if (graph.hasEdge(vertex1, vertex4) == false) {
                    ...
                    if ((total_gain - dist14) > 0) {
                        const std::vector<uint32_t> entry_vertex_indices = {vertex2, vertex3};
                        if (graph.hasPath(entry_vertex_indices, vertex1, this->improve_eps_, this->improve_k_).size() > 0 ||
                            graph.hasPath(entry_vertex_indices, vertex4, this->improve_eps_, improve_k_).size() > 0) {
     ...
     * - >1 It : (s) is missing an edge and (c) has one too much
     *
     * This method assumes the caller (wrapper for it==0, or the previous recursion step for it>0)
     * has already removed the current edge and installed self-loops for both endpoints, and supplied
     * the corresponding 'gain' (sum of removed-edge weights minus added-edge weights so far).
     *
     * On success it appends all performed changes to 'changes' and returns true.
    ...
    bool optEdge(std::vector<deglib::builder::BuilderChange>& changes,
                 ...
                 const uint8_t it) {
        auto& graph = this->graph_;
        const auto k = this->improve_k_;
        const auto eps = this->improve_eps_;
        if (k == 0) return false;

        const auto& fs = graph.getFeatureSpace();
        const auto dist_func = fs.get_dist_func();
        const auto dist_param = fs.get_dist_func_param();
        const auto edges_per_vertex = graph.getEdgesPerVertex();

        // Helper to get existing weight (if present) else compute
        auto get_weight = [&](uint32_t u, uint32_t v) -> float {
            const auto nidx = graph.getNeighborIndices(u);
            const auto nw = graph.getNeighborWeights(u);
            for (uint32_t i = 0; i < edges_per_vertex; ++i)
                if (nidx[i] == v) return nw[i];
        ...
        float best_score = std::numeric_limits<float>::lowest();
        {
            ...
            for (auto&& cand : topListAscending(rs_bc)) {
                ...
                const auto c_weights = graph.getNeighborWeights(c);
            ...
            // b: replace self-loop -> c
            graph.changeEdge(b, b, best_c, best_w_bc);
            changes.emplace_back(b, b, 0.f, best_c, best_w_bc);

            // c: replace d -> b
            graph.changeEdge(best_c, best_d, b, best_w_bc);
            changes.emplace_back(best_c, best_d, best_w_cd, b, best_w_bc);

            // d: replace c -> self-loop
            graph.changeEdge(best_d, best_c, best_d, 0.f);
            changes.emplace_back(best_d, best_c, best_w_cd, best_d, 0.f);
        }
     ...
     */
    bool optEdge(uint32_t a, uint32_t b, float dist_ab) {
        if (improve_k_ == 0) return false;

        auto& graph = this->graph_;
        std::vector<deglib::builder::BuilderChange> changes;
        ...
        if (optEdge(changes, a, b, a, a, dist_ab, 0) == false) {
            ...
            for (size_t i = 0; i < sz; ++i) {
                const auto& c = changes[sz - 1 - i];
                graph.changeEdge(c.internal_index, c.to_neighbor_index, c.from_neighbor_index, c.from_neighbor_weight);
            }
            return false;
        }
        return true;
    ...
    bool simpleEdgeSwaps(uint32_t vertex1, uint32_t vertex2, float dist12) {
        ...
        if (graph.hasEdge(vertex3, vertex1) || graph.hasEdge(vertex3, vertex2) || graph.hasEdge(vertex4, vertex1) ||
            graph.hasEdge(vertex4, vertex2)) {
            return false;
        }
    ...
    bool improveEdges() {
        ...
        for (size_t edge_idx = 0; edge_idx < edges_per_vertex; edge_idx++) {
            ...
            if (graph.hasEdge(vertex1, vertex2) &&
                deglib::analysis::checkRNG(graph, edges_per_vertex, vertex2, vertex1, neighbor_weights[edge_idx]) == false)
        ...
        /*for (size_t edge_idx = 0; success == false && edge_idx < edges_per_vertex; edge_idx++) {
          const auto vertex2 = neighbor_indices[edge_idx];
          if(graph.hasEdge(vertex1, vertex2) && deglib::analysis::checkRNG(graph, edges_per_vertex, vertex2, vertex1,
        neighbor_weights[edge_idx])) success |= improveEdges(vertex1, vertex2, neighbor_weights[edge_idx]);
            //success |= optEdge(vertex1, vertex2, neighbor_weights[edge_idx]);
            //success |= simpleEdgeSwaps(vertex1, vertex2, neighbor_weights[edge_idx]);
        }*/
    ...
    auto& build(std::function<void(deglib::builder::BuilderStatus&)> callback, const bool infinite = false) {
        ...
        do {
            ...
            if (this->new_entry_queue_.size() > 0 || this->remove_entry_queue_.size() > 0) {
                ...
                if (this->new_entry_queue_.size() > 0) add_task_manipulation_index = this->new_entry_queue_.front().manipulation_index;
                ...
                if (add_task_manipulation_index < del_task_manipulation_index) {
                    ...
                    while (this->new_entry_queue_.size() > 0 && batch.size() < this->extend_batch_size &&
                           this->new_entry_queue_.front().manipulation_index < del_task_manipulation_index) {
...
void remove_non_mrng_edges(deglib::graph::MutableGraph& graph) {
    ...
    std::cout << "Removed " << removed_rng_edges << " edges in " << duration_ms << " ms. Final graph contains "
              << deglib::analysis::calc_non_rng_edges(graph) << " non-RNG edges\n";
}
...
void optimize_edges(
    ...
    auto builder =
        deglib::builder::EvenRegularGraphBuilder(graph, rnd, deglib::builder::StreamingData_SchemeC, 0, 0.0f, k_opt, eps_opt, i_opt, 1, 0);
    ...
    const auto improvement_callback = [&](deglib::builder::BuilderStatus& status) {
        ...
        if (status.step % (iterations / 10) == 0) {
            ...
            std::cout << std::setw(7) << status.step << " step, " << std::setw(5) << duration << "s, AEW: " << std::fixed
                      << std::setprecision(2) << std::setw(4) << avg_edge_weight << ", " << (connected ? "" : "not") << " connected, "
