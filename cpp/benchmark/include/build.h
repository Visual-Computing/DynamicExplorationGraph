...
inline void create_graph(const deglib::StaticFeatureRepository& repository,
    ...
    auto builder = deglib::builder::EvenRegularGraphBuilder(
        graph, rnd, lid, k_ext, eps_ext, k_opt, eps_opt, i_opt, swap_tries, additional_swap_tries, use_rng, use_path_verification);
    ...
    const auto improvement_callback = [&](deglib::builder::BuilderStatus& status) {
        ...
        if (status.step % log_after == 0 || size == base_size) {
            ...
            log("{:7} vertices, {:8}ms, {:8} / {:8} improv, AEW: {:4.2f} -> Sorted:{:.1f}, InOrder:{:.1f}, {} connected & {}, RSS {} & "
                "peakRSS {}\n",
                size,
                duration,
                status.improved,
                status.tries,
                avg_edge_weight,
                fmt::join(weight_histogram_sorted, " "),
                fmt::join(weight_histogram, " "),
                connected ? "" : "not",
 ...
 */
inline std::vector<std::pair<std::string, uint32_t>> create_incremental_graphs(const deglib::StaticFeatureRepository& repository,
    ...
    log("Start graph builder (RNG pruning: {})\n", use_rng ? "enabled" : "disabled");
    auto builder = deglib::builder::EvenRegularGraphBuilder(
        graph, rnd, lid, k_ext, eps_ext, k_opt, eps_opt, i_opt, swap_tries, additional_swap_tries, use_rng);
    builder.setThreadCount(thread_count);
    if (thread_count == 1) builder.setBatchSize(1, 1);
    ...
    for (const auto& [graph_file, target_size] : result_files) {
        ...
        if (!std::filesystem::exists(graph_file)) graph.saveGraph(graph_file.c_str());
    }
 ...
 * @return Vector of pairs (graph_path, vertex_count) for each graph (existing or newly created).
 */
inline std::vector<std::pair<std::string, uint32_t>> reduce_graph_incremental(deglib::graph::SizeBoundedGraph& graph,
                                                                              const std::string& output_dir,
                                                                              const std::string& graph_name_base,
                                                                              const uint32_t step_size,
                                                                              const uint8_t k_opt,
    ...
    auto rnd = std::mt19937(7);
    auto builder = deglib::builder::EvenRegularGraphBuilder(
        graph, rnd, deglib::builder::OptimizationTarget::LowLID, 0, 0, k_opt, eps_opt, i_opt, 0, 0, use_rng);
    builder.setThreadCount(thread_count);
    if (thread_count == 1) builder.setBatchSize(1, 1);
 ...
 */
inline void measure_reduction_on_prebuilt_graphs(const std::string& input_dir,
                                                 const std::string& graph_name_base,
                                                 const uint32_t step_size,
 ...
 */
inline std::vector<std::pair<std::string, uint64_t>> improve_and_test(deglib::graph::SizeBoundedGraph& graph,
                                                                      const std::string& output_dir,
                                                                      const std::string& graph_name_base,
                                                                      const uint8_t k_opt,
                                                                      const float eps_opt,
