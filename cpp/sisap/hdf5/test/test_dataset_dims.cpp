// test_dataset_dims.cpp — Verify dimensions of EACH dataset in the HDF5 file
//
...
TEST(Hdf5DatasetDims, train) {
    ...
    EXPECT_EQ(it->second.num_rows, 200000u);
    EXPECT_EQ(it->second.num_cols, 1024u);
    EXPECT_EQ(it->second.element_size, 2u);
    EXPECT_EQ(it->second.total_bytes, 409600000u);
}

TEST(Hdf5DatasetDims, allknn_dists) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();
    ...
    EXPECT_EQ(it->second.total_bytes, 25600000u);
}

TEST(Hdf5DatasetDims, allknn_knns) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();
...
TEST(Hdf5DatasetDims, itest_dists) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();
...
TEST(Hdf5DatasetDims, itest_knns) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();
...
TEST(Hdf5DatasetDims, itest_queries) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();
...
TEST(Hdf5DatasetDims, otest_dists) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();

    auto datasets = scan_datasets(SISAP_H5);
    auto it = datasets.find("otest/dists");
    ASSERT_NE(it, datasets.end()) << "Dataset 'ottest/dists' not found";
    EXPECT_EQ(it->second.num_rows, 10000u);
    EXPECT_EQ(it->second.num_cols, 1000u);
    EXPECT_EQ(it->second.element_size, 4u);
    EXPECT_EQ(it->second.total_bytes, 40000000u);
...
TEST(Hdf5DatasetDims, otest_knns) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();
...
TEST(Hdf5DatasetDims, otest_queries) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();
...
TEST(Hdf5DatasetDims, ScanReturnsAll9Datasets) {
    if (!file_exists(SISAP_H5)) GTEST_SKIP();
