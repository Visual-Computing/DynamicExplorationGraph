 ...
 * 5. Converts transformed database vectors (d+1 dims) to FP16.
 * 6. Creates a ReadOnlyGraph using Metric::FP16L2 with the FP16 database,
 *    mapping the graph structure built in L2-space.
 * 7. Loads FP32 query vectors, appends 0.0f (d+1 dims), and converts to FP16.
 * 8. For each query, uses fp16_graph.search() with Metric::FP16L2.
    ...
    float flas_radius_decay = 0.93f)
{
    ...
    // --------------------------------------------------------------------------
    // Convert transformed features (d+1 dims) to FP16 and build ReadOnlyGraph (FP16L2)
    // --------------------------------------------------------------------------
    ...
    // --------------------------------------------------------------------------
    // Load query vectors, append 0.0f, and convert to FP16 (d+1 dims)
    // --------------------------------------------------------------------------
