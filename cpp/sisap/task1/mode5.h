    ...
    uint32_t prune_worst = 0)
{
    ...
    double t_reorder_start = sisap_common::now_ms();
    deglib::graph::ReadOnlyGraphExternal::reorder_features_inplace(graph, fp16_data.data(), dims);
    conversion_ms += sisap_common::now_ms() - t_reorder_start;

    double t_ext_start = sisap_common::now_ms();
    ...
    conversion_ms += sisap_common::now_ms() - t_ext_start;
