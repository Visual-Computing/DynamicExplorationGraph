...
TEST(Rerank, SingleQuerySingleCandidate) {
    ...
    auto results = deglib::search::rerank(fs, query.data(), 1, base.data(), 1, candidates, 1, 1, 1);
...
TEST(Rerank, SingleQueryMultipleCandidates) {
    ...
    auto results = deglib::search::rerank(fs, query.data(), 1, base.data(), 3, candidates, 3, 2, 1);
...
TEST(Rerank, MultipleQueries) {
    ...
    auto results = deglib::search::rerank(fs, queries.data(), 2, base.data(), 2, candidates, 2, 2, 1);
...
TEST(Rerank, KTopZeroReturnsAllCandidates) {
    ...
    auto results = deglib::search::rerank(fs, query.data(), 1, base.data(), 3, candidates, 3, 0, 1);
...
TEST(Rerank, KTopLargerThanCandidates) {
    ...
    auto results = deglib::search::rerank(fs, query.data(), 1, base.data(), 1, candidates, 1, 3, 1);
...
TEST(Rerank, NullQueriesThrows) {
    ...
    EXPECT_THROW(deglib::search::rerank(fs, nullptr, 1, nullptr, 1, candidates, 1, 1, 1), std::invalid_argument);
}
...
TEST(Rerank, NullCandidatesThrows) {
    ...
    EXPECT_THROW(deglib::search::rerank(fs, query.data(), 1, nullptr, 1, nullptr, 1, 1, 1), std::invalid_argument);
}
...
TEST(Rerank, InvalidCandidateIndexSkipped) {
    ...
    auto results = deglib::search::rerank(fs, query.data(), 1, base.data(), 1, candidates, 2, 2, 1);
...
TEST(Rerank, UsesQueriesAsTargetsWhenBaseNull) {
    ...
    auto results = deglib::search::rerank(fs, query.data(), 2, nullptr, 0, candidates, 2, 2, 1);
