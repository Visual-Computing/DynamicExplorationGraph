...
TEST(Filter, EmptyFilter) {
    ...
    deglib::search::Filter f(empty.data(), 0, 10, 10);
    EXPECT_EQ(f.size(), 0u);
...
TEST(Filter, SingleLabel) {
    int labels[] = {5};
    deglib::search::Filter f(labels, 1, 10, 11);
    EXPECT_EQ(f.size(), 1u);
...
TEST(Filter, BoundaryLabels) {
    int labels[] = {0, 10};
    deglib::search::Filter f(labels, 2, 10, 11);
    EXPECT_EQ(f.size(), 2u);
    EXPECT_TRUE(f.is_valid(0));
    EXPECT_TRUE(f.is_valid(10));
...
TEST(Filter, LargeMaxValue) {
    ...
    deglib::search::Filter f(labels, 5, 128, 129);
    EXPECT_EQ(f.size(), 5u);
    EXPECT_TRUE(f.is_valid(0));
...
TEST(Filter, InclusionRate) {
    int labels[] = {0, 1, 2};
    deglib::search::Filter f(labels, 3, 9, 10);
...
TEST(Filter, InclusionRateFull) {
    ...
    deglib::search::Filter f(labels.data(), 10, 9, 10);
    EXPECT_DOUBLE_EQ(f.get_inclusion_rate(), 1.0);
}

TEST(Filter, ForEachValidLabel) {
    int labels[] = {1, 3, 5};
    deglib::search::Filter f(labels, 3, 5, 6);
...
TEST(Filter, ForEachValidLabelEmpty) {
    std::vector<int> empty;
    deglib::search::Filter f(empty.data(), 0, 10, 11);
    int count = 0;
...
TEST(Filter, BitsetCorrectness) {
    ...
    deglib::search::Filter f(labels.data(), labels.size(), 127, 128);
    EXPECT_EQ(f.size(), 4u);
