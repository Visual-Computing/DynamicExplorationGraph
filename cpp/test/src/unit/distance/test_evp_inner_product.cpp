...
static std::pair<std::vector<std::byte>, std::vector<std::byte>> make_evp_pair(uint32_t dim, uint32_t non_zeros, int seed_a = 42, int seed_b = 99) {
    std::mt19937 rng_a(seed_a), rng_b(seed_b);
    std::vector<float> a(dim), b(dim);
    std::normal_distribution<float> dist(0.0f, 1.0f);
    for (auto& x : a) x = dist(rng_a);
    for (auto& x : b) x = dist(rng_b);
    return {deglib::quantization::evp::quantize_single(a.data(), dim, non_zeros), deglib::quantization::evp::quantize_single(b.data(), dim, non_zeros)};
}
...
TEST(EvpInnerProduct, NormalizationAndSymmetry) {
    ...
    float sim_self = deglib::distances::evp_ip::EvpInnerProduct::compare_naive(a.data(), a.data(), &dim);
    EXPECT_GT(sim_self, 0.0f);

    float sim_ab = deglib::distances::evp_ip::EvpInnerProduct::compare_naive(a.data(), b.data(), &dim);
    float sim_ba = deglib::distances::evp_ip::EvpInnerProduct::compare_naive(b.data(), a.data(), &dim);
    EXPECT_FLOAT_EQ(sim_ab, sim_ba);

    float expected_dist = 1.f - (sim_ab / (2.0f * dim));
    float dist = deglib::distances::evp_ip::EvpInnerProduct::compare(a.data(), b.data(), &dim);
...
TEST(EvpInnerProduct, Avx2MatchesNaive) {
    ...
    for (uint32_t dim : {64, 128, 256}) {
        ...
        float naive = deglib::distances::evp_ip::EvpInnerProduct::compare(a.data(), b.data(), &dim);
        float avx2 = deglib::distances::evp_ip::EvpInnerProduct_AVX2<deglib::distances::ResidualMode::Full>::compare(a.data(), b.data(), &dim);
...
TEST(EvpInnerProduct, Avx512MatchesNaive) {
    ...
    for (uint32_t dim : {64, 128, 256}) {
        auto [a, b] = make_evp_pair(dim, dim / 4);
        float naive = deglib::distances::evp_ip::EvpInnerProduct::compare(a.data(), b.data(), &dim);
        float avx512 = deglib::distances::evp_ip::EvpInnerProduct_AVX512<deglib::distances::ResidualMode::Full>::compare(a.data(), b.data(), &dim);
