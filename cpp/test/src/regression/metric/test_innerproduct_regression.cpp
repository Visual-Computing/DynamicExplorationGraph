...
TEST(DeglibRegressionIP, MultiInstructionSetBenchmark)
{
    ...
    if (deglib::cpu::has_avx512()) {
        run_regression_test("AVX512_16Ext", deglib::Metric::InnerProduct, 18000.0, 10.0, 0.774,
    ...
    if (deglib::cpu::has_avx2()) {
        run_regression_test("AVX2_16Ext", deglib::Metric::InnerProduct, 18000.0, 10.0, 0.774,
                            base_data.data(), query_data.data(), base_count, query_count, dim, gt_data,
                            deglib::distances::fp32_ip::InnerProductFloat16Ext_AVX2{}, 50);
    }
    if (deglib::cpu::has_sse42()) {
        run_regression_test("SSE_16Ext", deglib::Metric::InnerProduct, 15000.0, 11.5, 0.774,
                            base_data.data(), query_data.data(), base_count, query_count, dim, gt_data,
                            deglib::distances::fp32_ip::InnerProductFloat16Ext_SSE{}, 50);
    }
#endif
    run_regression_test("Scalar", deglib::Metric::InnerProduct, 11500.0, 15.4, 0.78,
                        base_data.data(), query_data.data(), base_count, query_count, dim, gt_data,
