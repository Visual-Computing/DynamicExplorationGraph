...
deglib::search::ResultSet read_only_graph_search_wrapper(
        ...
        const py::array_t<float, py::array::c_style> query, const float eps, const uint32_t k, const uint32_t max_distance_computation_count)
{
    py::buffer_info query_info = query.request();
    return graph.search(entry_vertex_indices, static_cast<std::byte*>(query_info.ptr), eps, k, max_distance_computation_count);
}
...
PYBIND11_MODULE(deglib_cpp, m) {
	...
	py::class_<deglib::graph::ReadOnlyGraph>(m, "ReadOnlyGraph")
		...
		.def("get_feature_space", [](const deglib::graph::ReadOnlyGraph& g) -> const deglib::SpaceInterface<float>& { return g.getFeatureSpace(); }, py::return_value_policy::reference)
