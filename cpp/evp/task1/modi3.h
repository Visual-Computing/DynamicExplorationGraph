    ...
    const std::string& output_path)
{
    ...
    std::printf("\n--- Exploration (EVP Build EVP Explore, max_dist=%u) ---\n", max_distance_count);
    ...
    uint32_t prune_worst = 0)
{
    ...
    std::printf("=== EVP Bits Graph Benchmark (Mode 3: EvpBuildEvpExplore) ===\n");
    std::printf("Data path: %ls\n", data_path.wstring().c_str());
    std::printf("NON_ZEROS=%u, K_TOP=%u, K_GRAPH=%u, K_EXT=%u, EPS_EXT=%.3f, MODE=build-evp-explore\n",
    ...
    // --------------------------------------------------------------------------
    std::printf("========================================================================\n");
    std::printf("  FINAL SUMMARY (EVP Build, EVP Explore - Mode 3)                       \n");
    std::printf("========================================================================\n");
    std::printf("Load Time:               %.2f ms\n", load_ms);
    std::printf("Quantize Time:           %.2f ms\n", quantize_ms);
