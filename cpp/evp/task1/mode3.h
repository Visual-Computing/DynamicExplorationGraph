...
/**
 * @file mode3.h
 * @brief Benchmark Mode 3: EVP Build, EVP Explore (evp-build-evp-explore)
 * 
 * Behavior:
 * 1. Quantizes training features to EVP bits representation.
 * 2. Builds a SizeBoundedGraph using the EvpBits metric.
 * 3. Explores the EVP graph topology directly using the Metric::EvpBits metric.
 * 4. No candidate reranking is performed.
 */
    ...
    uint32_t prune_worst = 0)
{
    ...
    std::printf("=== EVP Build, EVP Explore - Mode 3 ===\n");
    ...
    if (!loaded) {
        ...
        for (size_t start_row = 0; start_row < count; start_row += load_chunk_size) {
            ...
            // Quantize chunk
            double t_chunk_quant = evp_common::now_ms();
            auto quantized = deglib::quantization::quantize_batch(
                chunk_vectors, static_cast<uint32_t>(dims), non_zeros, threads);
            double chunk_quantize_ms = evp_common::now_ms() - t_chunk_quant;
            ...
            std::printf("  Chunk [%6zuk - %6zuk): Load = %.2fs, Quantize = %.2fs, Build = %.2fs | Elapsed = %.2fs\n", 
                        start_row / 1000, (start_row + current_chunk_size) / 1000, 
                        chunk_load_ms / 1000.0, chunk_quantize_ms / 1000.0, chunk_build_ms / 1000.0, elapsed_s);
        }
    ...
    std::printf("\n");
    double total_time_ms = load_ms + quantize_ms + build_ms + best_timings.explore_ms;

    evp_common::print_summary(
        "EVP Build, EVP Explore", 3,
