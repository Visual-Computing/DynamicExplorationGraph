...
/**
 * @file mode5.h
 * @brief Task 2 Mode 5: FP32 Build + FP32 Search mit FLAS-Vorsortierung
 *
 * Behavior:
 * 1. Loads FP32 training vectors from "train" as the database.
 * 2. Runs FLAS (Fast Linear Assignment Sorting) on all training vectors
 *    to produce a 1D ordering that places similar vectors nearby.
 * 3. Builds a SizeBoundedGraph using Metric::InnerProduct with
 *    FloatSpace(dims, Metric::InnerProduct), inserting vectors in
 *    FLAS-sorted order (external labels preserved = original indices).
 * 4. Loads FP32 query vectors from "queries" (separate dataset from database).
 * 5. For each query, uses graph.search() with entry point at vertex 0.
 * 6. Tracks build time, FLAS sort time, and search time separately.
    ...
    bool use_flas = false)
{
    ...
    if (!loaded) {
        ...
        for (size_t chunk_start = 0; chunk_start < count; chunk_start += load_chunk_size) {
            size_t current_chunk_size = std::min(load_chunk_size, count - chunk_start);
            ...
            for (size_t j = 0; j < current_chunk_size; ++j) {
                ...
                builder.addEntry(original_idx, std::move(feature));
            }
            ...
            std::printf("  Chunk [%6zuk - %6zuk): Build = %.2fs | Elapsed = %.2fs\n",
                        chunk_start / 1000, (chunk_start + current_chunk_size) / 1000,
