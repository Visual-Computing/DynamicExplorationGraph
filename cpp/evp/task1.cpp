...
int main(int argc, char* argv[]) {
    try {
        ...
        if (mode == "fp16-build-fp16-explore" || mode == "fp16" || mode == "modi1") {
            ...
            return task1::mode3::run(path, threads, non_zeros, k_graph, k_ext, eps_ext, k_top, max_distance_count, run_recall, output_path);
        } else if (mode == "evp-build-evp-explore-fp16-rerank" || mode == "evp" || mode == "modi4") {
            return task1::mode4::run(path, threads, non_zeros, k_graph, k_ext, eps_ext, k_top, max_distance_count, evpK, run_recall, output_path);
        } else if (mode == "evp-build-fp16-external-search" || mode == "modi5") {
            return task1::mode5::run(path, threads, non_zeros, k_graph, k_ext, eps_ext, k_top, max_distance_count, run_recall, output_path);
        } else if (mode == "evp-build-fp16-asymmetric-search" || mode == "evp-asymmetric" || mode == "modi6") {
            return task1::mode6::run(path, threads, non_zeros, k_graph, k_ext, eps_ext, k_top, max_distance_count, run_recall, output_path);
        } else if (mode == "evp-build-fp16-asymmetric-search-rerank" || mode == "evp-asymmetric-rerank" || mode == "modi7") {
            return task1::mode7::run(path, threads, non_zeros, k_graph, k_ext, eps_ext, k_top, max_distance_count, evpK, run_recall, output_path);
        } else {
