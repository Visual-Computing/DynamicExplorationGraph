...
static float test_approx_explore(
    const deglib::graph::InternalGraph& graph,
    ...
    deglib::concurrent::parallel_for(0, entry_vertex_indices.size(), threads, [&](size_t i, size_t thread_id) {
        ...
        if (result_queue.size() != k) {
            fmt::print(
                stderr, "Exploration with k={} got only {} results for query {} and max_distance_count {}\n", k, result_queue.size(), i, max_distance_count
            );
            abort();
        }
...
static std::vector<float> estimate_recall(
    const deglib::graph::InternalGraph& graph,
    const deglib::FeatureRepository& query_repository,
...
static void test_graph_explore(
    const deglib::graph::InternalGraph& graph,
    ...
    for (uint32_t f = 0; f <= explore_depth; f++, k_factor *= 10) {
        for (uint32_t i = (f == 0) ? 1 : 2; i < 11; i++) {
            ...
            if (linear_baseline_us > 0 && query_count > abort_sample_size) {
                ...
                if (sample_time_per_query > linear_baseline_us) {
                    log("max_distance_count {:5}, k {:4}, ABORTED ({}us/query > {}us baseline after {} queries)\n", max_distance_count, k,
                        sample_time_per_query, linear_baseline_us, sample_size);
                    return;
                }
            }
            ...
            for (size_t r = 0; r < repeat; r++)
                recall =
                    deglib::benchmark::test_approx_explore(graph, entry_vertex_indices, include_entry, ground_truth, k, max_distance_count, threads, filter);
            uint64_t search_time_us = stopw.getElapsedTimeMicro();
